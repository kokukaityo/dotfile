...
Go によるエンジン書き直しが進行中。既存の Bash コードと並行して開発する。
...
| ------------- | ---------- | -------------------------------- |
| Go            | 1.26+      | ビルド・テスト                   |
| gofumpt       | latest     | フォーマッター（gofmt の厳格版） |
| golangci-lint | latest     | リンター                         |
...
| コマンド     | 内容                               |
| ------------ | ---------------------------------- |
| `make build` | バイナリをビルド（`dist/dotfile`） |
...
- ~~Go によるエンジン書き直し（シングルバイナリ配布）~~ → 進行中（`cmd/dotfile/`）
- Homebrew tap
